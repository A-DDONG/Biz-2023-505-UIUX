...
// 자바스크립트에서 처음으로 사용하기 시작한 독특한 data 개체
...
document.addEventListener("DOMContentLoaded", () => {
  const join = () => {
    // #join_form 이 감싸고 있는 input 들을 모두 secelct 하여 join_inputs(배열)에 담아라
    ...
    const input_password = join_inputs[INPUT_INDEX.PASSWROD].value;
    if (!input_password) {
      alert("비밀번호는 필수 항목입니다");
      join_inputs[INPUT_INDEX.PASS].focus();
      return false;
    }
    ...
    if (input_password !== re_password) {
      ...
      join_inputs[INPUT_INDEX.PASSWROD].value = "";
      join_inputs[INPUT_INDEX.INDEX.RE_PASSWORD].value = "";
      join_inputs[INPUT_INDEX.PASSWROD].focus();
      return false;
    }
    ...
    if (!email_check.test(input_email)) {
      alert("이메일 형식이 잘못되었습니다(example@example.com)");
      join_inputs[INPUT_INDEX.EMAIL].focus();
      return false;
    }
